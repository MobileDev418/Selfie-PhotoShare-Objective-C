...
//#define kBaseLink @"http://192.168.1.103/SelfiusTest/"
#define kBaseLink @"http://selfius.com/"
//#define kBaseLink @"http://zzz.gadgyapp.com/"

#define kAPIKey @"1234567890"


//#define kAPIKey @"slakdjfalweiroqwetrsfas54dfa85sd7f8asdfasdf4q5wer"

#define kConnectionTimeout 300
...
- (void)GetPostsForUserID:(int)userID page:(int)page pageSize:(int)pageSize onCompletion:(void (^)(NSMutableArray *, ServerResponse))completion {
    ...
    if (userID < 1) {
        ...
        [httpClient POST:@"api/getUserPost" parameters:parameters success:^(AFHTTPRequestOperation *operation, id responseObject) {
            NSArray *rawPosts = [responseObject objectForKey:@"items"];
            ...
            [self debugger:parameters.description methodLog:@"api/getUserPost" dataLogFormatted:responseObject];
            completion(posts, OK);
        } failure:^(AFHTTPRequestOperation *operation, NSError *error) {
            [self debugger:parameters.description methodLog:@"api/getUserPost" dataLog:error.description];
            completion(nil, UNKNOWN_ERROR);
        }];
