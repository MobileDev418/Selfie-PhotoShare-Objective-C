...
- (IBAction)buttonPostTouchUpInside:(id)sender {
    ...
    if ([self.delegate respondsToSelector:@selector(showImageForPost:sender:)] && self.post.postImagePath.length) {
        ...
        NSLog(@"---------- touched image!!!");
        [EXPhotoViewer showImageFrom:self.imageViewPostImage];
    }
...
- (IBAction)buttonShareTouchUpInside:(id)sender{
    ...
    [[[UIActionSheet alloc] initWithTitle:@"Where are you going to share this image?" delegate:self cancelButtonTitle:@"Cancel" destructiveButtonTitle:nil otherButtonTitles:@"Facebook", @"Twitter", nil] showInView:self];
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex {
    ...
    if ([[actionSheet buttonTitleAtIndex:buttonIndex] isEqualToString:@"Facebook"]) {
        ...
        [self.delegate ShareToFacebook:self.imageViewPostImage sender:self];
        
    } else if ([[actionSheet buttonTitleAtIndex:buttonIndex] isEqualToString:@"Twitter"]) {
        NSLog(@"Twitter button clicked!.......");
        [self.delegate ShareToTwitter:self.imageViewPostImage sender:self];
    }
}
