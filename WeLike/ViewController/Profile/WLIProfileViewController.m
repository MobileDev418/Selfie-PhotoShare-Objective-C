...
- (void)viewDidLoad {
    ...
    self.buttonSearchUsers.hidden = YES;
    ...
    [self.collectionViewUserPost registerNib:nib forCellWithReuseIdentifier:@"WLIPostCollectionViewCell"];
    
    [self reloadDataOnSegment];
}
...
- (void)reloadDataOnSegment{
    
    loadingUserPosts = YES;
    [hud show:YES];
    [sharedConnect GetPostsForUserID:self.user.userID page:1 pageSize:kDefaultPageSize onCompletion:^(NSMutableArray *posts, ServerResponse serverResponseCode) {
        loadingUserPosts = NO;
        self.UserPosts = posts;
        [self.collectionViewUserPost reloadData];
        [hud hide:NO];
    }];
}
...
- (NSInteger)numberOfSectionsInCollectionView:(UICollectionView *)collectionView {
    return ceil(self.UserPosts.count / 3.0);
}
...
- (NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section {
    ...
    postsCount = (int)self.UserPosts.count;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.delegate = self;
    cell.post = self.UserPosts[indexPath.section * 3 + indexPath.row];
